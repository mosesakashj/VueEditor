<template>
  <div class='editor' ref='editor'  style="background-color: silver;height: 777px;width:100%; overflow: auto">
    ...
    <div v-if='overlay' class='overlays'>
      <div v-for='(page, page_no) in pages' class='overlay' :key='`${page.uuid}-overlay`' :ref='`${page.uuid}-overlay`'
        v-html='overlay(page_no+1, pages.length)' :style='page_style(page_no, false)'>
      </div>
    </div>
    ...
    <div class='content' ref='content' :contenteditable='editable' :style='page_style(-1)' @input='input' @keyup='process_current_text_style'>
      ...
      <div v-for='(page, page_no) in pages' class='page'
        ...
        :style='page_style(page_no, page.template ? false : true)' :contenteditable='!page.template ? true : false' >
...
export default {
  ...
  props: {
    ...
    page_format_mm: { type: Array, default: () => [210, 297] },

    // Page margins in CSS
    page_margins: { type: String, default: '10mm 15mm' },
  ...
  methods: {
    ...
    async reset_content () {
      ...
      if(!this.content.length) this.$emit('update:content', [''])
    ...
    async fit_content_over_pages () {
      ...
      for(let page_no = this.pages.length - 1; page_no >= 0; page_no--) {
        const page = this.pages[page_no]
        ...
        if(!page_elt || !document.body.contains(page_elt)) this.pages.splice(page_no, 1)
      }

      // If all the document was wiped out, start a new empty document
      if(!this.pages.length) this.reset_content()
      ...
      for(let page_no = 0; page_no < this.pages.length; page_no++) { // page length can grow inside this loop
        const page = this.pages[page_no]
        const page_elt = this.$refs[page.uuid][0]
        let next_page = this.pages[page_no + 1]
        ...
        if(!page.template && (prev_page_modified_flag || page_elt.innerHTML != page.prev_innerHTML
          ...
          if(page_elt.clientHeight <= this.pages_height && next_page && next_page.content_idx == page.content_idx) {
            ...
            if(!next_page_elt.childNodes.length) this.pages.splice(page_no + 1, 1)
          }
          ...
          if(page_elt.clientHeight > this.pages_height) {
            ...
            if(!next_page || next_page.content_idx != page.content_idx) {
              ...
              this.pages.splice(page_no + 1, 0, next_page)
              await this.$nextTick() // wait for DOM update
    ...
    page_style (page_no, allow_overflow) {
      ...
      if(this.display == "horizontal") {
        if(inner_width > (this.pages.length * page_with_plus_spacing)){
          ...
          left_px = inner_width / (nb_pages_x * 2) * (1 + page_no * 2) - page_width / 2
        } else {
          nb_pages_x = this.pages.length
          left_px = page_with_plus_spacing * page_no + page_width / 2 * (this.zoom - 1)
        }
        top_mm = 0
        ...
        page_column = (page_no % nb_pages_x)
        x_pos = inner_width / (nb_pages_x * 2) * (1 + page_column * 2) - page_width / 2
        x_ofx = Math.max(0, (page_width * this.zoom - inner_width) / 2)
        left_px = x_pos + x_ofx
        top_mm = ((this.page_format_mm[1] + page_spacing_mm) * this.zoom) * Math.floor(page_no / nb_pages_x)
    ...
    before_print () {
      ...
      for(const [page_no, page] of this.pages.entries()){
        const page_elt = this.$refs[page.uuid][0]
        ...
        page_clone.style.breakBefore = page_no ? 'page' : 'auto'
